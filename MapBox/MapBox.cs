...
namespace MapBox
{
    public partial class MapBox : UserControl
    {
        ...
        /// 录点相邻点最小显示间隔(mm)
        /// </summary>
        public int MinSpacing { get; set; } //50;
        ...
        /// 数据描述
        /// </summary>
        public Dictionary<int, string> IdDescDict { get; set; }
        ...
        ///     原始数据更新
        /// </summary>
        public void UpdateRaw(byte type, byte[] data)
        {
            try
            {
                if (type == 202) // 地图
                {
                    ...
                    var onePackLen = 20;
                    var count = data.Length / onePackLen;
                    for (var i = 0; i < count; i++)
                    {
                        var id = BitConverter.ToInt32(data, onePackLen * i);
                        var tag = BitConverter.ToInt32(data, onePackLen * i + 4);
                        var x = BitConverter.ToInt32(data, onePackLen * i + 8);
                        var y = BitConverter.ToInt32(data, onePackLen * i + 12);
                        var z = BitConverter.ToInt32(data, onePackLen * i + 16);
                ...
                else if (type == 201) // 定位
                {
                    ...
                    for (var i = 0; i < count; i++)
                    {
                        ...
                        var y = BitConverter.ToInt32(data, onePackLen * i + 12);
                        var z = BitConverter.ToInt32(data, onePackLen * i + 16);
                        var yaw = BitConverter.ToSingle(data, onePackLen * i + 20);
                        //if (id == 0)
                        //{
                        ...
                        //for(int k = 1; k < 30; k++)
                        //{
                        //    list.Add(new Node(id + k, tag, x, y, yaw));
                        //}
                    }
