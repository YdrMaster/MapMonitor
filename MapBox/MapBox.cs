...
namespace MapBox
{
    public partial class MapBox: UserControl
    {
        ...
        /// 录点相邻点最小显示间隔(mm)
        /// </summary>
        public int MinSpacing = 0;//50;
        // 地图
        private List<MapNode> mapList = new List<MapNode>();
        // 移动节点
        private List<PosNode> posList = new List<PosNode>();
        ...
        // 鼠标拖拽与缩放
        private Point mouseTranslate = new Point();
        private Point mouseLocation = new Point();
        private double mouseZoomK = 1;
        private double mouseZoomKLast = 1;
        private Point mouseZoomB = new Point();
        private Point mouseZoomBLast = new Point();
        ...
        public void UpdateRaw(byte[] data)
        {
            try
            {
                ...
                if (type == 202)        // 地图
                {
                    ...
                    int onePackLen = 20;
                    int count = (data.Length - 1) / onePackLen;
                    for (int i = 0; i < count; i++)
                    {
                        int id = BitConverter.ToInt32(data, onePackLen * i + 1);
                        int tag = BitConverter.ToInt32(data, onePackLen * i + 5);
                        int x = BitConverter.ToInt32(data, onePackLen * i + 9);
                        int y = BitConverter.ToInt32(data, onePackLen * i + 13);
                        int z = BitConverter.ToInt32(data, onePackLen * i + 17);
                ...
                else if (type == 201)   // 定位
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        int y = BitConverter.ToInt32(data, onePackLen * i + 13);
                        int z = BitConverter.ToInt32(data, onePackLen * i + 17);
                        float yaw = BitConverter.ToSingle(data, onePackLen * i + 21);
        ...
        public void UpdatePos(List<Node> nodeList)
        {
            Invoke(new UpdateData((nodes) => {
                foreach (Node node in nodes)
                {
                    ...
                    foreach (PosNode pos in posList)
                    {
                        if (pos.Id == node.Id)
                        {
                            posNode = pos;
                    ...
                    else                 // 有则更新
                    {
                        ...
                        if (record)
                        {
                            ...
                            for(int i = posNode.PosList.Count - 2; i >= 0; i--)
                            {
        ...
        private void ListView_AddItem(PosNode node)
        {
            ...
            string sId = node.Id.ToString();
            if (sId.Length > 2 && sId.Length % 2 == 1)
            {
                sId = "0" + sId;
            }
            item = new ListViewItem(new string[] { sId });
            item.Tag = node.Id;
        ...
        {
            // 自动变换适应窗口
            double x = (point.X - offset.X) * scale * 0.8 + 0.1 * Width;
            double y = Height - ((point.Y - offset.Y) * scale * 0.8 + 0.1 * Height);
            // 鼠标拖动与缩放
        ...
        private double CalcScale()
        {
            ...
            foreach(PosNode posNode in posList)
            {
                if (posNode.Visible)
                {
        ...
        private void CalcMinMax(List<Node> dataList, ref Point min, ref Point max)
        {
            ...
            {
                if (min.X > dataList[i].X)
                {
                    min.X = dataList[i].X;
                }
                if (max.X < dataList[i].X)
                {
                    max.X = dataList[i].X;
                }
                if (min.Y > dataList[i].Y)
                {
                    min.Y = dataList[i].Y;
                }
                if (max.Y < dataList[i].Y)
                {
                    max.Y = dataList[i].Y;
                }
            }
        ...
        private void MapBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (CalcScale() > 0) // 计算偏移缩放
            {
                ...
                List<Point> winMapList = new List<Point>();
                foreach (MapNode node in mapList)
                {
                    winMapList.Add(CalcWinCoord(node.Coordinate));
                }
                List<Point> winPosList = new List<Point>();
                foreach (PosNode node in posList)
                {
                    winPosList.Add(CalcWinCoord(node.Coordinate));
                }
                // 绘制地图与移动连线
                ...
                // 绘制地图
                for (int i = 1; i < winMapList.Count; i++)
                {
                    // 画线
                    graphics.DrawLine(Pens.Black, winMapList[i - 1], winMapList[i]);
                }
                for (int i = 0; i < winMapList.Count; i++)
                {
                    // 画点
                    Point pt = winMapList[i];
                ...
                {
                    if (!cbPoint.Checked && !posList[i].Visible)
                    {
                        continue;
                    }
                    Point pt = winPosList[i];
                    // 点
                    graphics.FillEllipse(
                        posList[i].Brush, pt.X - 10, pt.Y - 10, 20, 20);
                    // 偏航角
                    float yaw = posList[i].Yaw;
                    if (!float.IsNaN(yaw))
                    {
                        int x = (int)(50 * Math.Cos(yaw));
                        int y = -(int)(50 * Math.Sin(yaw));
                ...
                foreach (PosNode node in posList)
                {
                    if (node.Visible)
                    {
                        Point lastPt = new Point(-1, -1);
                        for (int i = 0; i < node.PosList.Count; i++)
                        {
                            if (node.PosList[i].Tag < 0)
                            {
                                continue;
                            }
                            Point pt = CalcWinCoord(node.PosList[i].Coordinate);
                                ...
                                node.PosList[i].Tag > trackBar1.Value)
                            {
                                ...
                                {
                                    graphics.DrawLine(Pens.Purple, pt, lastPt);
                                }
                                lastPt = pt;
                            }
                        }
                ...
                // 比例尺
                int rulerWidth = 50; // 比例尺显示长度(像素)
                string rulerText = "";
                double ruler = rulerWidth / (scale *0.8 * mouseZoomK);
                int nRuler = 1;
                while (ruler >= 10)
                {
                    ruler /= 10;
                ...
                graphics.DrawLine(Pens.Black, Width - 20 - rulerWidth, Height - 30, Width - 20 - rulerWidth, Height - 33);
                ...
                graphics.DrawString(rulerText, new Font("Arial", 10), Brushes.Black,
                    new RectangleF(Width - 40 - rulerWidth, Height - 30, 40 + rulerWidth, 20), sfCenter);
            }
        }
        ...
        private void MapBox_Resize(object sender, EventArgs e)
        {
            ...
            flowLayoutPanel1.Width = (listView1.Location.X - 30) > 400 ? 
                400 : (listView1.Location.X - 30);
        ...
        {
            ListViewHitTestInfo hinfo = listView1.HitTest(e.Location);
        ...
        private void listView1_ItemChecked(object sender, ItemCheckedEventArgs e)
        {
            ...
            foreach (PosNode node in posList)
            {
                if (node.Id == id)
                {
                    ...
                    if (same)
                    {
                        miHideAll.Text = e.Item.Checked ? "隐藏所有" : "显示所有";
                    }
                    break;
                }
            }
        ...
        private void ListView2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                int id = (int)listView2.SelectedItems[0].Tag;
                for(int i = posList.Count - 1; i >= 0; i--)
                {
                    if (posList[i].Id == id)
                    {
                        // 将选中的id移至列表最后(绘图时在最上层)
                        PosNode node = posList[i];
                        posList.Remove(node);
                        posList.Add(node);
                        Invalidate();
                        break;
        ...
        private void ListView1_ItemMouseHover(object sender, ListViewItemMouseHoverEventArgs e)
        {
            new ToolTip().SetToolTip(
                e.Item.ListView, MinSpacing.ToString() + "mm");
        }
        ...
        private void ListView2_ItemMouseHover(object sender, ListViewItemMouseHoverEventArgs e)
        {
            ...
            foreach(PosNode node in posList)
            {
                if (node.Id == id)
                {
                    ...
                    {
                        new ToolTip().SetToolTip(
                            e.Item.ListView, node.PosList.Count.ToString() + "点");
                    }
                    break;
                }
            }
        ...
        private void MapBox_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            // 更新B值
            double k = mouseZoomK / mouseZoomKLast;
        ...
        private void ListView1_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            new ToolTip().SetToolTip(listView1, MinSpacing.ToString() + "mm");
            // 根据最小间隔更新显示标志
            foreach (PosNode node in posList)
            {
                Node last = null;
                foreach(Node point in node.PosList)
                {
        ...
        {
            lblThr.Text = trackBar1.Value.ToString();
        }
    }
}
